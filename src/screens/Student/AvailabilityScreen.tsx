...
export const AvailabilityScreen: React.FC<AvailabilityScreenProps> = ({ navigation }) => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const pastDates = Array.from({ length: 7 }, (_, i) => format(subDays(today, i), 'yyyy-MM-dd'));
        ...
        {days.map((date, index) => {
          ...
          return (
            <View key={index} style={[styles.chip, { backgroundColor: chipColor }]}>
              <Text style={[styles.chipText, { color: textColor }]}>{dayName}</Text>
            </View>
          );
        })}
      </View>
    ...
    >
      ...
      <View style={styles.selectionRow}>
        ...
        >
          ...
          <Text style={[styles.cardText, isAvailable === true && { color: COLORS.success, fontWeight: 'bold' }]}>
            I am Available
          </Text>
        </TouchableOpacity>
        ...
        >
          <Text style={styles.cardIcon}>❌</Text>
          <Text style={[styles.cardText, isAvailable === false && { color: COLORS.error, fontWeight: 'bold' }]}>
            Not Available
          </Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  selectionRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: SPACING.xl, gap: SPACING.md },
  card: {
    flex: 1,
    backgroundColor: COLORS.surface,
    padding: SPACING.lg,
  ...
  cardSelectedAvailable: { borderColor: COLORS.success, backgroundColor: '#F1F8E9' },
  cardSelectedNotAvailable: { borderColor: COLORS.error, backgroundColor: '#FFEBEE' },
  cardIcon: { fontSize: 32, marginBottom: SPACING.sm },
  cardText: { fontSize: FONTS.md, color: COLORS.textSecondary, textAlign: 'center' },
  input: { marginBottom: SPACING.xl, backgroundColor: COLORS.surface },
  confirmButton: { borderRadius: 8, paddingVertical: SPACING.xs, marginBottom: SPACING.xl },
  historySection: { marginTop: SPACING.md, marginBottom: SPACING.xl },
  historyTitle: { fontSize: FONTS.lg, fontWeight: 'bold', color: COLORS.text, marginBottom: SPACING.md },
