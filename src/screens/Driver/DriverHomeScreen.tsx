...
import { View, StyleSheet, ScrollView, RefreshControl, TouchableOpacity, ActivityIndicator, Animated, Easing } from 'react-native';
...
const DriverHomeScreen: React.FC<DriverHomeScreenProps> = ({ navigation }) => {
  ...
  const [myRoute, setMyRoute] = useState<(Route & { routeId: string }) | null>(null);
  const [driverAvailabilityDoc, setDriverAvailabilityDoc] = useState<any>(null);
  const [todayRide, setTodayRide] = useState<(Ride & { rideId: string }) | null>(null);
    ...
    const fetchRouteAndRides = async () => {
      try {
        ...
        if (!routeSnap.empty) {
          ...
          const routeData = { routeId: routeDoc.id, ...routeDoc.data() } as Route & { routeId: string };
          ...
          if (studentIds.length > 0) {
            ...
            studentIds.forEach((sid: string) => {
              ...
              const unsub = onSnapshot(doc(db, COLLECTIONS.AVAILABILITY, docId), (docSnap) => {
                const data = docSnap.exists() ? docSnap.data() : null;
                todayStatusMap[sid] = data?.isAvailable === true;
                todayBoardedMap[sid] = data?.boarded === true;
                
                const availableCount = Object.values(todayStatusMap).filter(Boolean).length;
                const boardedCount = Object.values(todayBoardedMap).filter(Boolean).length;
          ...
          console.log('Starting ride listener for routeId:', routeDoc.id, 'date:', todayStr);
          ...
          const unsubRides = onSnapshot(ridesQuery, (snapshot) => {
            ...
            snapshot.forEach((d) => {
              ...
              if (rData.date === todayStr) {
                found = { rideId: d.id, ...rData } as Ride & { rideId: string };
              }
            });
  ...
  const handleStartRide = async () => {
    ...
    try {
      await updateDoc(doc(db, COLLECTIONS.RIDES, todayRide.rideId), { status: 'active' });
      navigation.navigate('ActiveRide', { rideId: todayRide.rideId, routeId: myRoute.routeId });
    } catch (error) {
      ...
      >
        ...
        <View style={styles.summaryCard}>
          <View style={styles.summaryItem}>
            ...
            <Text style={[styles.summaryNumber, { color: COLORS.primary }]}>{todayAvailableCount}</Text>
          ...
          <View style={styles.summaryItem}>
            <MaterialCommunityIcons name="map-marker-path" size={20} color="#16A34A" />
            <Text style={[styles.summaryNumber, { color: '#16A34A' }]}>{totalStudents}</Text>
          ...
          <View style={styles.summaryItem}>
            <MaterialCommunityIcons name="bus" size={20} color={COLORS.accent} />
            <Text style={[styles.summaryNumber, { color: COLORS.accent }]}>{todayRide ? '1' : '0'}</Text>
        ...
        <View style={styles.miniCardsRow}>
          ...
          <TouchableOpacity 
            style={[
              styles.splitCard,
              driverAvailabilityDoc === null ? styles.notMarkedCard :
              driverAvailabilityDoc.isAvailable ? styles.availableCard : styles.unavailableCard
            ]}
            onPress={() => navigation.navigate('DriverAvailability')}
          >
            <View style={styles.splitCardHeader}>
              <MaterialCommunityIcons 
                name={driverAvailabilityDoc === null ? "calendar-today" : driverAvailabilityDoc.isAvailable ? "calendar-check" : "close-circle"} 
                size={16} 
                color={driverAvailabilityDoc === null ? "#F59E0B" : driverAvailabilityDoc.isAvailable ? "#16A34A" : "#DC2626"} 
              />
              <Text style={[
                styles.splitCardTitle, 
                { color: driverAvailabilityDoc === null ? "#92400E" : driverAvailabilityDoc.isAvailable ? "#16A34A" : "#DC2626" }
              ]}>
                Availability
              </Text>
            </View>
            <Text style={[
              ...
              {driverAvailabilityDoc === null ? "Tap to mark" : driverAvailabilityDoc.isAvailable ? "Available" : "Not Available"}
            </Text>
            <Text style={styles.splitCardBottom}>
              {driverAvailabilityDoc === null ? "Setup today's status" : 
                driverAvailabilityDoc.isAvailable && driverAvailabilityDoc.vehicleAvailable ? "Vehicle Ready" : 
                driverAvailabilityDoc.isAvailable ? "Vehicle Issue" : "Backup Arranged"
              }
            </Text>
          </TouchableOpacity>
          ...
          >
            <View style={styles.splitCardHeader}>
              ...
              <Text style={[styles.splitCardTitle, { color: COLORS.primary }]}>My Route</Text>
            </View>
            <Text style={styles.splitCardRouteName} numberOfLines={1}>{myRoute?.routeName || 'No Route Yet'}</Text>
        ...
        <View style={styles.sectionHeader}>
          ...
          {todayRide && <Text style={styles.currentTimeText}>{currentTimeStr}</Text>}
        </View>
          ...
          {rideLoading ? (
            ...
            <View style={styles.rideScheduledCard}>
              ...
              <View style={styles.rideInfoRow}>
                ...
                <Text style={styles.rideInfoText}>{String(todayRide.departureTime)}</Text>
                <View style={styles.flexSpacer} />
                <MaterialCommunityIcons name="account-group" size={16} color={COLORS.textSecondary} />
                <Text style={styles.rideInfoText}>{todayBoardedCount} aboard</Text>
              </View>
              <TouchableOpacity style={styles.actionBtnPrimary} onPress={handleStartRide}>
            ...
            <View style={styles.rideActiveCard}>
              ...
              <TouchableOpacity style={styles.actionBtnGreen} onPress={handleResumeRide}>
                <Text style={styles.actionBtnText}>Resume</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <View style={styles.noRideCentered}>
              <MaterialCommunityIcons name="calendar-remove" size={40} color="#D1D5DB" />
