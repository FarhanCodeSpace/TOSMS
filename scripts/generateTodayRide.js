...
function parseTimeToTimestamp(dateString, timeString) {
  const timeMatch = String(timeString || "")
    .trim()
    .match(/^(\d{1,2})(?::(\d{2}))?\s*(AM|PM)$/i);
  const rideDate = new Date(`${dateString}T00:00:00`);

  if (!timeMatch) {
    rideDate.setHours(8, 0, 0, 0);
    return admin.firestore.Timestamp.fromDate(rideDate);
  }
...
async function generateRidesForToday() {
  try {
    ...
    if (routesSnap.empty) {
      console.log(
        "No routes found in the database. Please ensure routes are created first.",
      );
      process.exit();
    }
    ...
    for (const doc of routesSnap.docs) {
      ...
      const existingRideSnap = await db
        .collection("rides")
      ...
      if (!existingRideSnap.empty) {
        console.log(
          `A ride already exists for route "${routeData.routeName || routeId}" on ${todayStr}.`,
        );
        const existingRideDoc = existingRideSnap.docs[0];

        if (existingRideDoc.data().status !== "scheduled") {
          console.log(
            `--> Updating status from '${existingRideDoc.data().status}' to 'scheduled' for testing.`,
          );
        ...
        console.log(
          `Creating new scheduled ride for route "${routeData.routeName || routeId}" on ${todayStr}...`,
        );
        ...
        await db.collection("rides").add({
          ...
          date: todayStr,
          status: "scheduled",
          departureTime: parseTimeToTimestamp(
            todayStr,
            routeData.departureTime || "8:00 AM",
          ),
