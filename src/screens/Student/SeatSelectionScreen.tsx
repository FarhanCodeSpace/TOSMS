...
type SeatSelectionScreenProps = StackScreenProps<StudentHomeStackParamList, 'SeatSelection'>;
...
export const SeatSelectionScreen: React.FC<SeatSelectionScreenProps> = ({ route, navigation }) => {
  ...
  const getSeatStatus = (seatNum: number): 'available' | 'booked' | 'selected' => {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      if (error?.message === 'SEAT_TAKEN') {
        Alert.alert('Seat Taken', 'Seat was just booked! Please select another seat.');
    ...
    <View style={styles.container}>
              ...
              <View style={styles.seatGroup}>
                {row.slice(0, 2).map((seatNum) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={styles.seatLabel}>{getSeatLabel(seatNum)}</Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
      ...
      <View style={styles.bottomPanel}>
        <View style={styles.selectionRow}>
          ...
          <Text style={styles.selectionValue}>{selectedSeat ? `Seat ${selectedSeat}` : 'None'}</Text>
        </View>

        <Text style={[styles.selectionLabel, { marginBottom: 4 }]}>Pickup Stop:</Text>
        <View style={styles.pickerContainer}>
          ...
          >
            ...
            {ride.stops.sort((a, b) => a.order - b.order).map((stop, i) => (
              <Picker.Item key={i} label={stop.stopName} value={stop.stopName} />
            ))}
          </Picker>
        </View>
        ...
        <View style={styles.fareRow}>
          ...
          <Text style={styles.fareAmount}>PKR {(fareAmount || ride.farePerSeat)?.toLocaleString()}</Text>
        </View>

        <Button
          mode="contained"
          buttonColor={isConfirmEnabled ? COLORS.accent : '#BDBDBD'}
          textColor="white"
...
const styles = StyleSheet.create({
  ...
  busFront: { alignItems: 'center', paddingVertical: SPACING.md, backgroundColor: '#E8F4F8', marginBottom: SPACING.md },
  steeringWheel: { fontSize: 40 },
  driverLabel: { color: COLORS.textSecondary, fontSize: FONTS.sm, fontWeight: 'bold' },
  ...
  selectionValue: { fontSize: FONTS.lg, fontWeight: 'bold', color: COLORS.primary },
  pickerContainer: {
    borderWidth: 1,
    borderColor: '#DEDEDE',
    borderRadius: 8,
