...
const CreateRideScreen: React.FC<CreateRideScreenProps> = ({ navigation }) => {
  ...
  const [totalSeats, setTotalSeats] = useState(
    currentUser?.vehicleCapacity?.toString() || "15",
  );
  ...
  const handleReverseGeocode = async (
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permission Denied",
          "Location permission is required to find addresses.",
        );
        return;
      }
  ...
  const addStop = () => {
    if (newStop.trim()) {
      setStops([
        ...stops,
        { stopName: newStop.trim(), order: stops.length + 1 },
      ]);
      setNewStop("");
    }
  };
  ...
  const renderStep1 = () => (
    <View>
      <TextInput
        label="Route Name (e.g., Rawalpindi to Islamabad)"
        value={routeName}
      ...
      <TextInput
        ...
        value={startLoc.name}
        onChangeText={(t) => setStartLoc((p) => ({ ...p, name: t }))}
        mode="outlined"
        style={styles.input}
      />
      <MapView
      ...
      <TextInput
        ...
        value={endLoc.name}
        onChangeText={(t) => setEndLoc((p) => ({ ...p, name: t }))}
        mode="outlined"
        style={styles.input}
      />
      <MapView
  ...
  const renderStep2 = () => (
    <View>
      ...
      <View style={styles.btnRow}>
        <TouchableOpacity
          style={[styles.flexBtn, styles.outlineBtn]}
          onPress={() => setStep(1)}
          activeOpacity={0.85}
        >
  ...
  const renderStep3 = () => (
    <View>
      ...
      <View style={styles.btnRow}>
        ...
        <TouchableOpacity
          style={[styles.flexBtn, isLoading && styles.postBtnDisabled]}
        ...
        >
          {isLoading ? (
            ...
            <Text style={styles.nextBtnText}>Post Ride</Text>
          )}
        </TouchableOpacity>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  nextBtnText: { color: "#FFFFFF", fontWeight: "700" },
  pickerTrigger: {
    padding: 16,
    backgroundColor: "white",
    borderRadius: 4,
